...
global home globalparams exptparams HW StopExperiment
StopExperiment = 0;
...
while ContinueExp == 1
    ...
    %% Repetition loop
    iRep = 0;
    while iRep < exptparams.Repetition;
        iRep = iRep+1;
        if ~ContinueExp,
            break;
        end
...
%                 datawrite(globalparams.datafilename,ResponseData,HW.params.fsAI);
%             end
        ...
        if ~strcmpi(exptparams.BehaveObjectClass,'Passive')
            if isfield(exptparams,'Figure')
                outfile=strsep(globalparams.mfilename,'\');
                exptparams.ResultsFigure=exptparams.Figure;
                SaveBehaviorFigure(exptparams,outfile{4});
            end
        end
    end
    %% Pause switch
    ...
    if ~isempty(exptparams.Repetition) && iRep==exptparams.Repetition && ...
            ContinueExp && ~exptparams.ContinuousTraining
        ContinueExp = ContinueOrNot;
    end
end
