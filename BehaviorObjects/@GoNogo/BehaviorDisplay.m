...
t=EarlyWin(1):BinSize:exptparams.ResponseWin(2);
TarRange=get(exptparams.TrialObject,'PrimaryHandle');
TarRange = get(TarRange,'TarRange');
tar=FirstResponse(FirstResponse(:,2)==TarRange,1);
...
if ~isempty(tar) && sum(~isnan(tar)) > 0
    [h2_Tar,xi] = ksdensity(tar+EarlyWin(1),t,'function','pdf','bandwidth',.2);
    plot(t,(h2_Tar./sum(h2_Tar)),'color',[1 0 0],'linewidth',2);
    hold on;
    lstr='Hit ';
    aa=axis;
...
if ~isempty(nontar) && sum(~isnan(nontar)) > 0
    [h2_NonTar,xi] = ksdensity(nontar+EarlyWin(1),t,'function','pdf','bandwidth',.2);
    plot(t,(h2_NonTar./sum(h2_NonTar)),'color',[.1 .5 .1],'linewidth',2);
    hold on;
    lstr=[lstr;'Fa  '];
    aa=axis;
    text(aa(2)-.25,.5.*aa(4),['n=' num2str(size(exptparams.AverageResponse.nontar,1))],'color',[.1 .5 .1],'fontsize',8);
else
    h1=hist([],t);
end
if ~isempty(probe) && sum(~isnan(probe)) > 0
    [h2_Probe,xi] = ksdensity(probe+EarlyWin(1),t,'function','pdf','bandwidth',.2);
    plot(t,(h2_Probe./sum(h2_Probe)),'color',[.1 .5 .5],'linewidth',2);
    hold on;
    lstr=[lstr;'pHit'];
    aa=axis;
    text(aa(2)-.25,.25.*aa(4),['n=' num2str(size(exptparams.AverageResponse.probe,1))],'color',[1 .5 .5],'fontsize',8);
else
    h1=hist([],t);
end
...
plot(NonTarAvgResp,'color',[.1 .5 .1]);
hold on;
plot(TarAvgResp,'color',[1 0 0]);
...
for cnt1 = 1:length(StimEvents)
    ...
    if strcmpi(Type,'Stim')
        stilen(1) = get(PrimaryHandle,'PreStimSilence');
        stilen(2) = get(PrimaryHandle,'PreStimSilence') + get(PrimaryHandle,'Duration');
        stilen(3) = get(PrimaryHandle,'PreStimSilence') + get(PrimaryHandle,'Duration') + get(PrimaryHandle,'PostStimSilence');
        ResponseWin= get(o,'ResponseWindow');
        EarlyWin = stilen(1)+[EarlyWin(1) EarlyWin(2)];
    end
end
plot(stilen([1 1 2 2])*fs,[0 0.6 0.6 0]*max(ylim),'--','color','b','LineWidth',2);
ht=title('Average Response','fontsize',8);
aa=axis;
set(ht,'Position',[400,.75.*aa(4)]);
if sum(abs(get(o,'EarlyWindow'))) > 0
    set(addgrid(0,fs*EarlyWin,'c','--'),'linewidth',2);
end
set(addgrid(0,fs*ResponseWin,'g','--'),'linewidth',2);
set(gca,'fontsize',8);
...
hh0=hist(FirstResponse(FirstResponse(:,2)>=0,3),stimnum);  %Total trial
hh0(hh0==0)=1;
hh=hist(FirstResponse(FirstResponse(:,2)>=0 & FirstResponse(:,4)==1,3),stimnum);  %Response during respwin
...
if ~isempty(FirstResponse(FirstResponse(:,2)==-1,3)) %Add probe performance
    ...
    xtl=[xtl probenum];
    xlabel('          Primary Set      ----------       Probe Set','fontsize',8);
else
    xlabel('Primary Set','fontsize',8);
end
ht=title('Response Rate','fontsize',8);
aa=axis;
set(ht,'Position',[.4*aa(2),.8*aa(4)]);
