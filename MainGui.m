...
%% This is the first GUI to popup in Psignal. It is where the global parameters are defined for the experiment.
gui_Singleton = 1;
...
%Read the settings from last time Psignal was used on this rig. All animal-specific settings are stored in each animal's folder.
...
if exist([roothome '\RecentMainGuiSettings.mat'],'file')
    ...
    if isfield(MainGuiSettings,Rig)
        set(handles.DataPath,'String',MainGuiSettings.(Rig).DataPath);
...
if strcmpi(get(handles.User,'String'),'User') || strcmpi(get(handles.User,'String'),'Animal')|| strcmpi(get(handles.User,'String'),'Weight')
...
global datapath
if ~isempty(get(handles.DataPath,'String'))
...
if exist([datapath '\' Animal '\HealthChart.mat'],'file')
    ...
    [AX,H1,H2] = plotyy(1:size(HealthData,1),cell2mat(HealthData(:,1)), 1:size(HealthData,1),cell2mat(HealthData(:,2)));
    H1.Marker='s';
    H2.Marker='s';
    H1.MarkerSize=10;
    ...
    if exist([datapath '\' Animal '\AnimalInfo.mat'],'file')==2
        ...
        hold on
        plot([1 size(HealthData,1)],[str2num(AnimalInfo.PullWeight) str2num(AnimalInfo.PullWeight)],'r--','LineWidth',2);
        set(AX(1),'ylim',[10 40])
...
temp.Device = t2{t1};
temp.WaterTrigger=1;
if  ~strcmpi(temp.Device,'OSIO') && ~isfield(handles,'WaterHW')
...
if ~strcmpi(Animal,'animal')
    ...
    HealthChart = [HealthChart; {str2num(handles.globalparams.Weight)} {str2num(handles.globalparams.HealthRating)} {handles.globalparams.Date}];
